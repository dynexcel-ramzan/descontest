...
class OracleSettingConnector(models.Model):
    ...
    def action_attendance_process(self):
        ...
        for u_att in user_attendance:
            ...
            today_att=self.env['hr.attendance'].search([('att_date','=',u_att.attendance_date),('employee_id','=',u_att.employee_id.id),('check_out','=',False)], limit=1)
            yesterday_att=self.env['hr.attendance'].search([('att_date','=',yesterday_date),('employee_id','=',u_att.employee_id.id),('check_out','=',False)], limit=1)
    ...
    def action_get_attendance_data(self):
        ...
        conn = cx_Oracle.connect('xx_odoo/xxodoo123$@//10.8.8.191:1521/PROD')
        cur = conn.cursor()
        statement = "select p.att_time AS timestamp, p.mac_number AS machine, p.card_no AS card, p.att_date AS attendance_date, p.creation_date AS creation_date, p.remarks AS remarks, p.updation_date AS updation_date from attend_data p where p.creation_date>=sysdate-6  and p.mac_number not in (48,49)"
        cur.execute(statement)
        attendances = cur.fetchall()  
        for attendance in attendances:
            ...
            duplicate_attendance = user_attendance.search([('card_no','=',attendance[2]),('time','=',attendance[0]),('attendance_date','=',ebs_timestamp.strftime('%Y-%m-%d'))], limit=1)
    ...
    def action_get_missing_attendance_data_a(self):
        ...
        for attendance in attendances:
            ...
            duplicate_attendance = user_attendance.search(
                [('card_no', '=', attendance[2]), ('time', '=', attendance[0]),
                 ('attendance_date', '=', ebs_timestamp.strftime('%Y-%m-%d'))], limit=1)
            if not duplicate_attendance:
                employee = self.env['hr.employee'].search([('barcode', '=', attendance[2])], limit=1)
                timestamdata = attendance[6]
                timestamp1 = timestamdata.strftime("%Y-%m-%d %H:%M:%S")
    ...
    def action_get_missing_attendance_data_b(self):
        user_attendance = self.env['hr.user.attendance']
        attendance_ids = []
        conn = cx_Oracle.connect('xx_odoo/xxodoo123$@//10.8.8.191:1521/PROD')
        cur = conn.cursor()
        statement = "select p.att_time AS timestamp, p.mac_number AS machine, p.card_no AS card, p.att_date AS attendance_date, p.creation_date AS creation_date, p.remarks AS remarks, p.updation_date AS updation_date from attend_data p where p.creation_date>=sysdate-15 and p.creation_date<=sysdate-9  and p.mac_number not in (48,49)"
        cur.execute(statement)
        attendances = cur.fetchall()
        for attendance in attendances:
            ebs_timestamp1 = attendance[6].strftime("%Y-%m-%d %H:%M:%S")
    ...
    def action_get_missing_attendance_data_c(self):
        user_attendance = self.env['hr.user.attendance']
        attendance_ids = []
        conn = cx_Oracle.connect('xx_odoo/xxodoo123$@//10.8.8.191:1521/PROD')
        cur = conn.cursor()
        statement = "select p.att_time AS timestamp, p.mac_number AS machine, p.card_no AS card, p.att_date AS attendance_date, p.creation_date AS creation_date, p.remarks AS remarks, p.updation_date AS updation_date from attend_data p where p.creation_date>=sysdate-20 and p.creation_date<=sysdate-14  and p.mac_number not in (48,49)"
        cur.execute(statement)
        attendances = cur.fetchall()
        for attendance in attendances:
            ebs_timestamp1 = attendance[6].strftime("%Y-%m-%d %H:%M:%S")
