...
class HrAttendanceReport(models.AbstractModel):
    ...
    def _get_report_values(self, docids, data=None):
        ...
        for employee11 in docs.employee_ids:
            work_day_line = [] 
            attendances = []
            leave_color = rectify_color = gazetted_color = rest_day = remarks = ' '
            ...
            statement = """select e.name as employee, p.check_in as checkin, p.check_out checkout, 
                           p.att_date as attendance_date, s.name as shift, p.worked_hours as hours from hr_attendance as p
                           left join hr_employee as e on p.employee_id=e.id
                           left join resource_calendar as s on p.shift_type_id=s.id
                           where p.employee_id="""+str(employee.id)+""" 
                        """
#             and  p.att_date BETWEEN """+str(docs.start_date)+""" and """+str(docs.end_date)+"""
            self.env.cr.execute(statement)
            emp_att = self.env.cr.dictfetchall()
            for att_line in emp_att:
                attendances.append({
                    'date': att_line['attendance_date'].strftime('%d-%b-%Y'),
                    'day':  att_line['attendance_date'].strftime('%A'),
                    'check_in': att_line['checkin'].strftime('%d-%b-%Y %H:%M:%S') if att_line['checkin'] else ' ',
                    'check_out':  att_line['checkout'].strftime('%d-%b-%Y %H:%M:%S') if att_line['checkout'] else ' ',
                    'hours': att_line['hours'],
                    'shift': att_line['shift'],
                    'leave': leave_color,
