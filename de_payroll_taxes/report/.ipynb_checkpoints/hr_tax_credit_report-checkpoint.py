...
class PayrollTaxCreditReport(models.AbstractModel):
    ...
    def generate_xlsx_report(self, workbook, data, lines):
        ...
        sheet.write(1,2, 'Reason' ,bold)
        sheet.write(1,3, 'Tax Credit Type' ,bold)
        sheet.write(1,4, 'Post',bold)
        sheet.write(1,5, 'Current Amount',bold)
        sheet.write(1,6, 'Period Name',bold)
        sheet.write(1,7, 'Dist Month',bold)
        row = 3
        for line in lines: 
            ...
            sheet.write(row, 4, line.credit_type, format1)  
            sheet.write(row, 5, line.post, format1)
            sheet.write(row, 6, line.tax_amount, format1) 
            sheet.write(row, 7, line.dist_month, format1) 
            row += 1           